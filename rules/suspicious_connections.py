...
def detect(packets: list[PacketRecord], flows: list[ConnectionFlow]) -> list[RuleAlert]:
    ...
    for pkt in packets:
        pair_times[(pkt.src_ip, pkt.dst_ip)].append(datetime.fromisoformat(pkt.timestamp))
    ...
    for flow in flows:
        if (
            flow.total_bytes > 10_000_000
            and _is_internal_ip(flow.src_ip)
            and not _is_internal_ip(flow.dst_ip)
        ):
            alerts.append(
                RuleAlert(
